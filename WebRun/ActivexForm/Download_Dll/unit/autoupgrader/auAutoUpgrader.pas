...
{$ENDIF}
  ...
  public
    ...
    property OnFinishUpgrade: TNotifyEvent read  FOnFinishUpgrade write FOnFinishUpgrade;
  published
    // derived properties
    property Proxy;
...
uses   Consts, SysUtils, auUtils,
     ...
     auAutoUpgraderPassword,wxoUnit;
...
//全江修改增加
begin
    if self.FCanExit then  FOnFinishUpgrade(self);
end;
  ...
  begin
      ...
      begin
        if FVersionControl = byNumber then
         ...
         if (self.FDownloadQueue.Count>0) then
            result:=true
          else
            result:=false;
  end;
  ...
  // retrieving the files to upgrade (#urlX)
   ...
   //没有升级文件
     ...
     if (FDownloadQueue.Count=0) then
      begin
        if Assigned(FOnEndUpgrade) then FOnEndUpgrade(Self);
          self.FCanExit:=true;
     end;
  ...
  if IsUpdateAvailable and (FDownloadQueue.Count <> 0) then
   ...
   begin
    ...
    if not FIsAutoCheck and (mNoUpdateAvailable in FShowMessages) then
      ...
      if (self.FCanExit=true) then  DoUpgrade(False);   //全江修改 ,执行完线程后退出
   end;
end;
  ...
  if FirstFile then // if first file then displaying the Wizard
   begin
    ...
    if FWizard.Enabled then
      ...
      WizardForm.Parent:=TForm(self.owner);
  ...
  if FCurrentFileIndex < FDownloadQueue.Count then
   begin
    ...
    // convert to temporary/output filename
     //--------------全江修改 begin----------------
      if (FRestartParams='/afterupgrade') then
           FCurrentFileName := GetCurrentDir+'\' +
                          URLToFileName(FCurrentFileName) + TMPExt
      else
          FCurrentFileName :=GetCurrentDir+'\' +
                          URLToFileName(FCurrentFileName) ;

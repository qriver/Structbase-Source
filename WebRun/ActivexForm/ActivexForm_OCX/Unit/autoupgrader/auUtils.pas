...
{$ELSE}
function InternetCrackUrl(lpszUrl: PAnsiChar; dwUrlLength, dwFlags: DWORD;
...
begin
  ...
  with URLComp do
   begin
    ...
    {$IFDEF UNICODE}
    Result := InternetCrackUrl(PWideChar(URL), Length(URL), ICU_ESCAPE, URLComp);
    {$ELSE}
    Result := InternetCrackUrl(PAnsiChar(URL), Length(URL), ICU_ESCAPE, URLComp);
    {$ENDIF}
